...
namespace ORB_SLAM2_MapReuse
{
    ...
    public:
        ...
        // Returns the camera pose (empty if tracking fails).
        cv::Mat TrackRGBD(const cv::Mat &im, const cv::Mat &depth, const double &timestamp);
        ...
        bool SaveMapUsingBoost(const std::string &filename);

        bool LoadMapUsingBoost(const std::string &filename);
