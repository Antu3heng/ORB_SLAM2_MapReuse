...
{
    ros::init(argc, argv, "EuRocStereo");
    ros::start();

    if(argc != 4)
    {
        cerr << endl << "Usage: rosrun ORB_SLAM2 EuRocStereo path_to_vocabulary path_to_settings do_rectify" << endl;
        ros::shutdown();
        return 1;
    }    
    ...
    SLAM.SaveTrajectoryKITTI(TrajKittiSavePath);
